...
- **Per-root index shards** under `<state-dir>/projects/<project>/roots/`,
  ...
  Writes are serialised and carry both a content fingerprint of the store and
  the time their scan began, so a slower scan cannot replace a newer snapshot
  and a *deletion* still propagates — sharding removes that race between
  instances, but one instance runs several processes. Merged at read time with a total ordering
