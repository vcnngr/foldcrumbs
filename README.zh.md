...
端点（与提供蒸馏服务的同一批服务器：MLX、Ollama、llama.cpp、LM Studio），并按
**排名**而非分数融合两个通道（倒数排名融合，`1/(60+rank_lex) + 1/(60+rank_sem)`）：
词法比率与余弦相似度是不同量纲，绝不作为数值相互比较 — 准入仍按分数（同义转述
需要跨过与从前相同的证据门槛），排序变为序数的。向量可以挽救词法看不到的同义
转述；精确词匹配保持词法第 1 名（多个精确匹配之间由决胜链排序），而两个通道相互印证
会影响排序 — 在明确的情况下胜出，但交叉排名的对手可能打平（由决胜链裁定），印证并不
保证在所有配置下都能反超。
开关关闭 — 或端点无响应 — 时，融合阶段被完全绕过，召回与纯词法逐字节一致。
...
**按项目、即时生成。**与其他所有命令一样，仪表盘读取的是你运行它的那个项目的
存储（在项目 A 里运行 `foldcrumbs dashboard` 就显示项目 A 的记忆）。没有任何
服务器，也不做缓存：每次运行都根据存储在那一刻的内容现算页面。如何阅读每个
面板 — 主视觉区、健康徽章、便当格 — 有一份独立指南：
[docs/dashboard.zh.md](docs/dashboard.zh.md)（[English](docs/dashboard.md)、
[Italiano](docs/dashboard.it.md)）。

页面顶部是一个**实时主视觉区**：记忆总数、活跃数、被召回数、联邦根，以及一个
心跳 — 其节奏来自存储真实的召回活动：用得多的存储比沉睡的存储跳得更快（CSS
动画，仍然无脚本），另有一个健康徽章（current / needs a sweep / attention），由
decay、expiry 与 conflicts 计算得出。其下是便当格（bento）布局：decay 清扫将会
归档的内容、被取代链、联邦根（shard 时效、条目数）、召回强化、最新记忆、信任度
直方图、抗腐化（预算、handoff 时效、语义通道）— 另有 **Expiry** 与
