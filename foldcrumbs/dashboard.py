...
h1{font-size:16px;letter-spacing:.12em;color:#7ee787;margin:0 0 4px}
.sub{color:#8b949e;font-size:12px;margin-bottom:20px}
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:14px}
.panel{background:#12161b;border:1px solid #21262d;border-radius:8px;padding:14px}
.panel h2{font-size:11px;letter-spacing:.18em;color:#58a6ff;margin:0 0 10px;
text-transform:uppercase}
.big{font-size:28px;color:#e6edf3}
.dim{color:#8b949e}
...
.bar{display:inline-block;height:10px;background:#238636;border-radius:2px;
vertical-align:middle}
.warn{color:#d29922}
.empty{color:#8b949e;font-style:italic}
.tag{display:inline-block;background:#1f6feb22;color:#79c0ff;border-radius:4px;
padding:0 6px;margin-left:6px;font-size:11px}
footer{margin-top:20px;color:#8b949e;font-size:11px}
"""
...
def render(data: dict) -> str:
    ...
    # STORE
    pairs = [(k, str(v)) for k, v in sorted(by.items())]
    panels.append(f"""<div class="panel"><h2>Store</h2>
<div class="big">{st['total']}</div><div class="dim">memories on disk</div>
<table>{_rows(pairs)}</table>
<p class="dim" style="word-break:break-all">{html.escape(st['dir'])}</p></div>""")

    # DECAY — what the sweep would archive right now
    cands = data["decay"]["candidates"]
    if cands:
        body = "".join(
            f"<tr><td>{_file_link(c['name'])}</td><td>{'expired' if c['name'] in data['decay']['expired'] else c['trust']}</td></tr>"
            for c in cands)
        panels.append(f"""<div class="panel"><h2>Decay</h2>
<div class="big warn">{len(cands)}</div>
<div class="dim">would be archived by <code>decay --apply</code></div>
<table>{body}</table></div>""")
    else:
        panels.append("""<div class="panel"><h2>Decay</h2>
<div class="big">0</div><div class="dim">nothing to archive — the store is current</div></div>""")

    # PRUNING — superseded chains awaiting prune
    chains = data["superseded"]
    if chains:
        body = "".join(
            f"<tr><td>{_file_link(c['old'])} → {_file_link(c['new']) if c['found'] else html.escape(c['new'])}</td><td></td></tr>"
            for c in chains)
        panels.append(f"""<div class="panel"><h2>Superseded</h2>
<div class="big">{len(chains)}</div>
<div class="dim">replaced memories kept on disk (<code>prune</code> clears)</div>
<table>{body}</table></div>""")
    else:
        panels.append("""<div class="panel"><h2>Superseded</h2>
<div class="big">0</div><div class="dim">no retired memories pending prune</div></div>""")

    # PARALLEL — federation roots
    roots = data["roots"]
    if roots:
        lines = []
        for r in roots:
            tag = '<span class="tag">this instance</span>' if r["current"] else ""
            entries = "" if r["entries"] is None else f"{r['entries']} entries"
            age = ("" if r["shard_age_days"] is None
                   else f'<br><span class="dim">shard {r["shard_age_days"]}d old</span>')
            lines.append(
                f"<tr><td>{html.escape(r['label'])}{tag}<br>"
                f"<span class=\"dim\">{html.escape(r['path'])}</span></td>"
                f"<td>{entries}{age}</td></tr>")
        panels.append(f"""<div class="panel"><h2>Federation</h2>
<div class="big">{len(roots)}</div><div class="dim">registered roots</div>
<table>{''.join(lines)}</table></div>""")
    else:
        panels.append("""<div class="panel"><h2>Federation</h2>
<div class="big">0</div><div class="dim">no roots registered</div></div>""")

    # ASYNC — reinforcement
    rf = data["reinforcement"]
    if rf["top"]:
        body = "".join(f"<tr><td>{_file_link(t['name'])}</td><td>{t['count']}</td></tr>"
                       for t in rf["top"])
        panels.append(f"""<div class="panel"><h2>Recall</h2>
<div class="big">{rf['total_recalls']}</div>
<div class="dim">total recalls · {rf['never_recalled']} memories never recalled</div>
<table>{body}</table></div>""")
    else:
        panels.append(f"""<div class="panel"><h2>Recall</h2>
<div class="big">{rf['total_recalls']}</div>
<div class="dim">no recalls recorded yet</div></div>""")
    ...
    # RECENT — the browsable face of the store
    if data["recent"]:
        body = "".join(
        ...
        panels.append(f"""<div class="panel"><h2>Latest memories</h2>
<table>{body}</table></div>""")
    ...
    bars = "".join(
        f'<div><span class="dim">{lo}–{hi}</span> '
        f'<span class="bar" style="width:{int(b / peak * 180)}px"></span> '
        f'<span>{b}</span></div>'
    ...
    panels.append(f"""<div class="panel"><h2>Trust</h2>{bars}
<table style="margin-top:8px">{types}</table></div>""")
    ...
    sem_line = (f"<tr><td>semantic channel</td>"
                f"<td>{'on' if sem['on'] else 'off'} · cache {sem['cache']}</td></tr>"
                if sem else "")
    panels.append(f"""<div class="panel"><h2>Anti-rot</h2><table>{_rows([
    ...
    # EXPIRY (only once the feature exists and has something to show)
    exp = st.get("expiry")
    if exp:
        nxt = (f"{_file_link(exp['next'][0])} on {exp['next'][1][:10]}"
               if exp["next"] else "—")
        panels.append(f"""<div class="panel"><h2>Expiry</h2><table>{_rows([
            ('lapsed', str(len(exp['lapsed']))),
            ('next to expire', nxt),
        ])}</table></div>""")

    # CONFLICTS (feature-detected)
    cq = data.get("conflicts")
    if cq is not None and any(cq.values()):
        panels.append(f"""<div class="panel"><h2>Conflicts</h2><table>{_rows([
            ('ambiguous pairs', str(cq['flagged'])),
            ('claims out', str(cq['claims_out'])),
            ('contested here', str(cq['contested_here'])),
        ])}</table>
<div class="dim" style="margin-top:6px"><code>foldcrumbs conflicts</code></div></div>""")

    return f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>foldcrumbs dashboard</title>
<style>{_CSS}</style></head><body>
<h1>FOLDCRUMBS // MEMORY DASHBOARD</h1>
<div class="sub">generated {html.escape(data['generated_at'])} — every number
computed live from the store; file names link to the actual memory files</div>
<div class="grid">{''.join(panels)}</div>
<footer>foldcrumbs — one folder of typed memory, no engine, no vector DB</footer>
</body></html>"""
